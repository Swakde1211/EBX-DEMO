...
public class LdapDirectory implements ExternalDirectory
{
		...
		LdapDirectoryConfig ldapConfig)
	{
		...
		catch (Exception e)
		{
			if (debugEnabled)
			{
				// removed Stack trace from logging since this is an "expected exception" in order to reduce log volume
				logger.debug("Search Exception:" + e.getMessage());
			}
			return null;
		}
		return res;
		...
		LdapDirectoryConfig ldapConfig)
	{
		...
		try
		{
			...
			if (debugEnabled)
			{
				logger.debug("Attributes found for" + name);
			}
		...
		String originalLogin)
	{
		...
		if (debugEnabled)
		{
			logger.debug("String used for EBX User search " + filter.toString());
		}
		...
		if (res == null || res.size() == 0)
		{
			if (debugEnabled)
			{
				logger.debug("EBX User " + login + " not found.");
			}
			return null;
		}
		...
		try
		{
			...
			if (attributes != null)
			{
				...
				if (debugEnabled)
				{
					logger.debug("EBX User Found.  Login: <" + login + ">, LdapName: <" + user.getLdapName() + ">, userIdAttributeValue: <" + userId + ">");
				}
	...
	public Boolean isUserInRole(final User user, final String roleId, final String roleLabel)
	{
		...
		try
		{
			...
			if (cachedRole == null || cachedRole.isExternal() == null)
			{
				...
				if (res == null || res.size() == 0)
				{
					...
					cachedRole = DirectoryCacheManager.cacheRole(roleId, extRoleId, Boolean.FALSE);
					return null;
				}
				else
				{
					// if group found - caching the result.
					cachedRole = DirectoryCacheManager.cacheRole(roleId, extRoleId, Boolean.TRUE);
					if (isBuiltIn)
					{
						return Boolean.TRUE;
					}
	...
	public Map<Path, String> updateUserProfile(final User ebxUser, final Adaptation user)
	{
		...
		{
			logger.debug(
				"Attributes were empty. Retrieving attributes from external diretory (LDAP).");
