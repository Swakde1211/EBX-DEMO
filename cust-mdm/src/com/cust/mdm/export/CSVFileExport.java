...
public class CSVFileExport extends ScriptTaskBean {
	...
	private static AdaptationHome toDataspace(final Repository repository, final String dataspaceName)
		...
		try {
			...
			final UserMessage message = Messages.get(CSVFileExport.class, Severity.ERROR,
	...
	private static AdaptationTable toTable(final Adaptation dataset, final String tablePath) throws OperationException {
		try {
			...
			final UserMessage message = Messages.get(CSVFileExport.class, Severity.ERROR, "TheProvidedTablePathIsIncorrect{0}",
					tablePath);
			throw OperationException.createError(message);
		}
	}
	...
	private static File toFile(final String filePath) throws OperationException {
		try {
			...
			final UserMessage message = Messages.get(CSVFileExport.class, Severity.ERROR, "TheProvidedFilePathIsIncorrect{0}",
					filePath);
			throw OperationException.createError(message);
		}
	}
