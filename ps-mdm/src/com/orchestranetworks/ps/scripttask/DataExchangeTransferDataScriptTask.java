 ...
 * <pre>{@code
		<bean className="com.orchestranetworks.ps.scripttask.DataExchangeTransferDataScriptTask">
			...
			<properties>
		          ...
		          <property name="sourceTablesTableFilterPredicates" input="true">
		              <documentation xml:lang="en-US">
		                  <label>Source Table, Table Filter Predicates</label>
		                  <description>
		                     Colon delimited for Source Table and Table Filter Predicate and semicolon delimiter for each pair		
		                     /root/sourceTable1:XPathFilterPredicate1;/root/sourceTable2:XPathFilterPredicate2
		                  </description>
		              </documentation>
		          </property>
...
public class DataExchangeTransferDataScriptTask extends ScriptTaskBean
{
	 ...
	 * example: 
	 * /root/sourceTable1:XPathFilterPredicate1;/root/sourceTable2:XPathFilterPredicate2
	 * /root/StagingSupplier:date-equal(src_Last_Updated_Date, '2022-01-26')
	 */
	private String sourceTablesTableFilterPredicates;

	private String delimiter = DEFAULT_DELIMITER;
	private String tableFilterPredicateDelimiter = DEFAULT_TABLE_FILTER_PREDICATE_DELIMITER;
	...
	private static final String DEFAULT_TABLE_FILTER_PREDICATE_DELIMITER = ":";
	...
	public void executeScript(ScriptTaskBeanContext context) throws OperationException
	{
		try
		{
			...
			// parse the SourceTables and its Predicates
			Map<String, String> parsedSourceTablesTableFilterPredicates = getParsedSourceTablesTableFilterPredicates();
				...
				.iterator(); iter.hasNext();)
			{
				...
				// add TableFilter to the SourceTable if a predicate is provided
				if (parsedSourceTablesTableFilterPredicates != null
					&& !parsedSourceTablesTableFilterPredicates.isEmpty())
				{
					String predicate = parsedSourceTablesTableFilterPredicates.get(entry.getKey());
					if (predicate != null)
					{
						TableFilter tableFilter = new TableFilter();
						tableFilter.setPredicate(predicate);
						sourceTableFilters.put(srcEBXTable, tableFilter);
					}
				}
	...
	private Map<String, String> getParsedSourceTablesTableFilterPredicates()
	{
		Map<String, String> parsedSourceTablesTableFilterPredicates = new HashMap<>();

		if (StringUtils.isEmpty(sourceTablesTableFilterPredicates))
		{
			return parsedSourceTablesTableFilterPredicates;
		}

		String[] sourceTableTableFilterPredicatePairs = sourceTablesTableFilterPredicates
			.split(tablePairDelimiter);
		if (sourceTableTableFilterPredicatePairs != null)
		{
			for (int i = 0; i < sourceTableTableFilterPredicatePairs.length; i++)
			{
				String[] sourceTableTableFilterPredicatePair = sourceTableTableFilterPredicatePairs[i]
					.split(tableFilterPredicateDelimiter);
				if (sourceTableTableFilterPredicatePair != null
					&& sourceTableTableFilterPredicatePair.length == 2)
				{
					String sourceTablePath = sourceTableTableFilterPredicatePair[0];
					String tableFilterPredicate = sourceTableTableFilterPredicatePair[1];
					parsedSourceTablesTableFilterPredicates
						.put(sourceTablePath, tableFilterPredicate);
				}
			}
		}
	...
	public String getSourceTablesTableFilterPredicates()
	{
		return sourceTablesTableFilterPredicates;
	}

	public void setSourceTablesTableFilterPredicates(String sourceTablesTableFilterPredicates)
	{
		this.sourceTablesTableFilterPredicates = sourceTablesTableFilterPredicates;
	}
