...
/**
 * A class comprised of static utility methods for dealing with Adaptations.
 * Most of the methods act on a record <code>Adaptation</code> or a record
 * <code>ValueContext</code>.
 */
public class AdaptationUtil {
	/**
	 * Given a record <code>Adaptation</code> and a <code>Locale</code>, this
	 * function will produce a string representation of the record.
	 */
	...
	/**
	 * Given a record <code>Adaptation</code>, this function will produce a string
	 * representation of the record using the default locale.
	 */
	public static final UnaryFunction<Adaptation, String> GetDefaultLabel = GetLabel.bindRight(Locale.getDefault());

	/**
	 * Given a <code>SchemaNode</code> and a <code>Locale</code>, this function will
	 * produce a string representation of the record.
	 */
	...
	/**
	 * Function to replace ${path} occurrences in an xpath predicate for a
	 * particular instance. If the path points to an instance the formatted primary
	 * key is used. TODO: compare with
	 * {@link #computeValuedPredicate(String, ValueContextForValidation)} for
	...
	public static String calculateLocalValues(final ValueContext vc, final String predicate) {
		...
		for (int i = 0; i < len; i++) {
			...
			try {
				...
				// log error
				VM.log.kernelWarn("Exception determining contextual xPath predicate: " + e.getLocalizedMessage());
				return null;
			}
		}
		return parsed;
	...
	/**
	 * Given a record <code>Adaptation</code> and a path representing an
	 * association/select node field of that record, return the
	 * <code>RequestResult</code> that can be used to iterate over the related
	 * records.
	 * 
	...
	public static void deleteLinkedRecordList(Adaptation record, Path path, ProcedureContext pContext)
			throws OperationException {
	...
	/**
	 * Get a list of values from a selection node or association by following a path
	 * within that table, or a list of records if no attribute path is specified
	 *
	 * @param record        the record
	 * @param path          the path of the selection node or association
	 * @param attributePath the path of the attribute on the selection node or
	 *                      association table
	 ...
	 * @param attributePath the path of the attribute on the selection node or
	 *                      association table
	 * @param distinct      exclude duplicates
	 * @param sortField     on which to sort the related records
	 * @param ascending     sort type
	 * @return the list of values or records, or an empty list
	 */
	public static List<?> getLinkedRecordList(Adaptation record, Path path, Path attributePath, boolean distinct,
			Path sortField, boolean ascending) {
	 ...
	 * @param sortField     on which to sort the related records
	 * @param ascending     sort type
	 * @param exludeNulls   when attributePath is specified, this will exclude nulls
	 *                      from the list when true
	 * @return the list of values or records, or an empty list
	 */
	public static List<?> getLinkedRecordList(Adaptation record, Path path, Path attributePath, boolean distinct,
			Path sortField, boolean ascending, boolean excludeNulls) {
	...
	/**
	 * Get a list of primary keys as strings from a selection node or association by
	 * following a path within that table,
	 *
	 * @param record the record
	 * @param path   the path of the selection node or association
	 ...
	 * @return the list of values, or an empty list
	 */
	public static List<String> getLinkedRecordKeyList(Adaptation record, Path path, boolean distinct, Path sortField,
			boolean ascending) {
		RequestResult reqRes = linkedRecordLookup(record, path);
		try {
			if (reqRes == null || reqRes.isEmpty()) {
	...
	/**
	 * Determine if there are any related records to the given record related by the
	 * path representing an association or selection node.
	 *
	 * @param record the record
	 * @param path   the path of the selection node or association
	...
	/**
	 * Get first record from a selection node or association (useful for singletons)
	 *
	 * @param record the record
	 * @param path   the path of the selection node or association
	...
	/**
	 * Get first value from a selection node or association by following a path
	 * within that table, or a the record if no attribute path is specified (useful
	 * for singletons)
	 *
	 * @param record        the record
	 * @param path          the path of the selection node or association
	 ...
	 */
	public static Object getFirstRecordFromLinkedRecordList(Adaptation record, Path path, Path attributePath) {
		RequestResult reqRes = linkedRecordLookup(record, path);
		try {
			if (reqRes == null || reqRes.isEmpty()) {
				return null;
	...
	/**
	 * Given a parameterized predicate string (where parameters are of the form
	 * ${pathString}, and using the provided value context, replace the parameters
	 * with the values of the paths specified.
	 *
	 * @param predicate parameterized predicate string
	 * @param vc        value context
	 * @return compiled predicate string
	 */
	public static String computeValuedPredicate(final String predicate, final ValueContextForValidation vc) {
		final Pattern pattern = Pattern.compile("\\$\\{(.*?)\\}");
		final Matcher matcher = pattern.matcher(predicate);
		String computedPredicate = predicate;
		while (matcher.find()) {
			final String predicatePart = matcher.group().replaceAll("\\$\\{", "").replaceAll("\\}", "");
			String predItem = "";
	...
	/**
	 * Given a record <code>Adaptation</code> and a path representing a foreign key
	 * field, return the target table of the foreign key.
	 *
	 * @param record the record
	...
	/**
	 * Given a record <code>ValueContext</code> and a path representing a foreign
	 * key field, return the target table of the foreign key.
	 *
	...
	/**
	 * Given a record <code>Adaptation</code> and a path representing a foreign key
	 * field, return the target record specified by the value of the foreign key
	 * field.
	 *
	 * @param record the record
	 * @param fkPath the path of the foreign key field
	...
	/**
	 * Given a record <code>ValueContext</code> and a path representing a foreign
	 * key field, return the target record specified by the value of the foreign key
	 * field.
	 *
	 * @param valueContext the record context
	 * @param fkPath       the path of the foreign key field
	...
	/**
	 * Given a record <code>Adaptation</code>, a path representing a foreign key
	 * field, and a path representing a field on the target record, return the value
	 * of that field on the related record.
	 *
	 * @param record          the record
	 * @param fkPath          the path of the foreign key field
	 ...
	 */
	public static Object followFK(final Adaptation record, final Path fkPath, final Path fkAttributePath) {
	...
	/**
	 * Given a record <code>ValueContext</code>, a path representing a foreign key
	 * field, and a path representing a field on the target record, return the value
	 * of that field on the related record.
	 *
	 * @param valueContext    the record context
	 ...
	 * @return Object the value of the field on the related record
	 */
	public static Object followFK(final ValueContext valueContext, final Path fkPath, final Path fkAttributePath) {
	...
	/**
	 * Given a record <code>Adaptation</code> and a path representing a repeating
	 * foreign key field, return the list of target records specified by the value
	...
	/**
	 * Due to a bug in 5.7.1, linked records returned from a tableRef do not respect
	 * the order specified for the repeating field. This method is used to work
	 * around this defect. Either a source record or record context should be passed
	 * in.
	 * 
	 * @param fkNode
	 * @param record
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static List<Adaptation> getLinkedRecords(SchemaNode fkNode, Adaptation record, ValueContext vc) {
		if (vc == null && record != null)
			vc = record.createValueContext();
		// todo: when fixed, simply: return
		// fkNode.getFacetOnTableReference().getLinkedRecords(vc);
	...
	/**
	 * Given a record <code>ValueContext</code> and a path representing a repeating
	 * foreign key field, return the list of target records specified by the value
	 * of the foreign key field.
	 *
	 * @param valueContext the record context
	...
	/**
	 * Given a record <code>Adaptation</code>, a path representing a repeating
	 * foreign key field, and a path representing a field on the target record,
	 * return the list of the values of that field on the related records.
	 *
	 * @param record          the record
	 * @param fkPath          the path of the repeating foreign key field
	 * @param fkAttributePath the path of the field on the target record
	 * @param includeNulls    indicator of whether the resulting collection should
	 *                        include null values
	 * @return values of the field on the related records
	 */
	public static List<?> followFKList(final Adaptation record, final Path fkPath, final Path fkAttributePath,
	...
	/**
	 * Given a record <code>ValueContext</code>, a path representing a repeating
	 * foreign key field, and a path representing a field on the target record,
	 * return the list of the values of that field on the related records.
	 *
	 * @param valueContext    the record context
	 ...
	 * @return values of the field on the related records
	 */
	public static List<?> followFKList(final ValueContext valueContext, final Path fkPath, final Path fkAttributePath,
			final boolean includeNulls) {
		final List<Adaptation> targetList = followFKList(valueContext, fkPath);
		return getValuesForFKList(targetList, fkAttributePath, includeNulls);
	}

	private static List<?> getValuesForFKList(List<Adaptation> targetList, Path fkAttributePath, boolean includeNulls) {
	 ...
	 *
	 * @deprecated Use {@link #getRecord(String, Adaptation, boolean, boolean)}
	 *             instead. It allows you to specify whether the xpath contains all
	 *             of the pk fields, which will result in quicker lookup time. This
	 *             method will assume <code>false</code> for backwards
	 *             compatibility, but it should be a conscious decision by the
	 *             caller.
	 * @param instance       the data set
	 * @param xPathPredicate the xpath
	 */
	@Deprecated
	public static Adaptation getOccurrenceFromXPathExpression(final Adaptation instance, final String xPathPredicate) {
		...
		if (table == null)
			return null;
		final String predicate = xPathPredicate.substring(indexOfOpeningBracket + 1, lenghtOfXPathPredicate - 1);
	...
	/**
	 * Given a value context, return the record for which the context holds proposed
	 * values. Note well: {@link ValueContext#getAdaptationInstance()} cannot be
	 * used since it returns the data set.
	 *
	...
	/**
	 * Looks up the given record again, by its primary key. This can be used for
	 * when you have a record that may have been updated by another process and you
	 * want to look it up again so that it is up to date (i.e. the data isn't
	 * stale).
	 * 
	 * @param record the record to reload
	 * @return the reloaded record
	 */
	// TODO: You should be able to just call record.getUpToDateInstance() and we can
	// deprecate this method.
	// That's equivalent to calling
	// record.getHome().findAdaptationOrNull(record.getAdaptationName()).
	public static Adaptation reloadRecord(Adaptation record) {
		return record.getContainerTable().lookupAdaptationByPrimaryKey(record.getOccurrencePrimaryKey());
	}

	/**
	 * Given a field context (e.g. on a constraint) and a path to another field on
	 * the same record, return the value of the other field, specified by its
	 * adaptation-relative path.
	 * 
	 * @param fieldContext
	 ...
	 */
	public static Adaptation getRecordFromOtherDataSpace(Adaptation record, AdaptationHome otherDataSpace) {
	...
	/**
	 * Get the same record from the initial snapshot data space, if it exists and
	 * restore the current record to the values from the initial snapshot record If
	 * it does not exist, then it must have been created in the current dataspace,
	 * so it will now be deleted
	 *
	 * @param record           the current record to be restored record to look for
	 * @param procedureContext the current procedureContext
	 * @return the restored record from the initial snapshot data space, or null of
	 *         the record was created in the current dataspace
	 * @throws OperationException
	 * @throws
	 */
	public static Adaptation restoreRecordFromInitialSnapshot(Adaptation currentRecord,
			ProcedureContext procedureContext) throws OperationException {
		if (currentRecord == null) {
			return null;
		}
		Adaptation initialSnapshotRecord = getRecordFromInitialSnapshot(currentRecord);
		if (initialSnapshotRecord != null) {
			return procedureContext.doModifyContent(currentRecord, initialSnapshotRecord.createValueContext());
		} else {
	 ...
	 * @throws OperationException if an error occurs, or if not found and
	 *                            <code>errorIfNotFound</code> is <code>true</code>
	 */
	@Deprecated
	public static Adaptation getRecord(String recordXpath, Adaptation dataSet, boolean errorIfNotFound)
			throws OperationException {
	 ...
	 * @param recordXpath           the xpath
	 * @param dataSet               the data set
	 * @param checkActualPrimaryKey Whether the xpath specifies each member of the
	 *                              primary key, for faster lookup
	 * @param errorIfNotFound       Whether to consider not found to be an error
	 * @throws OperationException if an error occurs, or if not found and
	 *                            <code>errorIfNotFound</code> is <code>true</code>
	 */
	...
	/**
	 * Return a user message with the specified message and severity where the
	 * message is prefixed by information about the record.
	 *
	 * @param record   the record
	 ...
	 */
	public static UserMessageString createUserMessage(Adaptation record, String msg, Severity severity) {
	...
	/**
	 * Gets all of the tables for a data set. This is equivalent of
	 * <code>getAllTables(dataSet, dataSet.getSchemaNode())</code>
	 *
	 * @param dataSet the data set
	 ...
	 * @return the tables for the data set under the given node, or an empty list if
	 *         none exist
	 */
	 ...
	 *
	 * @param adaptation         the record or data set (for cases where you want a
	 *                           data set level field)
	 * @param fieldPath          the path of the field within the given adaptation
	 * @param session            the user's session
	 * @param includeGroupLabels Include the labels of the parent group(s) of the
	 *                           field
	 * @return the label
	 */
	public static String getFieldLabel(Adaptation adaptation, Path fieldPath, Session session,
	...
	/**
	 * Depending on the schema type associated with the specified node, values will
	 * be quoted in predicates. For example, all the string types and date types
	 * would have their values in quotes.
	 * 
	...
	public static DifferenceBetweenOccurrences getRecordDifferencesInChildDataSpace(Adaptation record,
		...
		if (initialSnapshot == null) {
			throw OperationException
					.createError("No initial snapshot found for data space " + childDataSpace.getKey().getName());
		}
		Adaptation dataSet = record.getContainer();
		Adaptation initialDataSet = initialSnapshot.findAdaptationOrNull(dataSet.getAdaptationName());
		if (initialDataSet == null) {
			throw OperationException.createError("Data set " + dataSet.getAdaptationName().getStringName()
		...
		if (initialTable == null) {
			throw OperationException.createError("Table " + table.getTablePath().format() + " not found in data set "
		...
		if (initialRecord == null) {
			throw OperationException.createError("Record " + record.toXPathExpression() + " not found in snapshot "
					+ initialSnapshot.getKey().getName());
		}
	...
	/**
	 * Return the association record identified by the two records it associates and
	...
	public static Adaptation getAssociationRecord(Path tablePath, Adaptation key1, Adaptation key2) {
		AdaptationTable assocTable = key2.getContainerTable().getContainerAdaptation().getTable(tablePath);
		PrimaryKey key = assocTable.computePrimaryKey(
				new Object[] { key1.getOccurrencePrimaryKey().format(), key2.getOccurrencePrimaryKey().format() });
		return assocTable.lookupAdaptationByPrimaryKey(key);
	}

	/**
	 * Evaluate a 'path' expression where each of the paths except the last item
	 * represents an adaptation path against the adaptation represented by the
	 * previous path and the first path is one against the provided record.
	 * 
	 * @param recordContext
	 * @param paths
	...
	/**
	 * Starting from either a recordRoot (Adaptation) or a contextRoot
	 * (ValueContext), find the related records, a list of Adaptations, resulting
	 * from the provided Path. The Path should represent a fk, fk list, association,
	 ...
	 */
	public static List<Adaptation> evaluateSingle(Adaptation recordRoot, ValueContext contextRoot, Path path) {
		SchemaNode recordNode = recordRoot != null ? recordRoot.getSchemaNode() : contextRoot.getNode();
		...
		if (tableRef != null) {
			...
			return linkedRecord == null ? Collections.<Adaptation>emptyList() : Collections.singletonList(linkedRecord);
		} else if (fkNode.isAssociationNode() || fkNode.isSelectNode()) {
			if (recordRoot == null)
				recordRoot = getRecordForValueContext(contextRoot);
			return recordRoot != null ? getLinkedRecordList(recordRoot, path) : Collections.<Adaptation>emptyList();
		}
	...
	/**
	 * Using the list of adaptations resulting from the paths against the record,
	 * produce the list of field values for those records.
	 * 
	 * @param recordContext
	 * @param paths
	 * @param field
	 ...
	 */
	public static List<Object> evaluatePath(ValueContext recordContext, Path[] paths, Path field, boolean skipNull) {
	 ...
	 * @return the list of values for the path
	 */
	public static List<Object> evaluatePath(Adaptation record, Path[] paths, Path field, boolean skipNull) {
	...
	/**
	 * Answers whether the specified node represents a relationship field (foreign
	 * key, association or selection node).
	 *
	 * @param node the node
	...
	/**
	 * For a given node, if the node is a relationship node (foreign key,
	 * association or selection node), return the target table of the relationship.
	 * 
	 * Note that if this is called during schema compilation (such as from a setup
	 * method), it is not guaranteed that link properties will have been determined
	 * by then and therefore, you shouldn't rely on this returning a value in those
	 * cases.
	 *
	 * @param node the node
	 * @return the table node of the related table or null if the node is not a
	 *         relationship node
	 */
	...
	public static SchemaNode getTableNodeForRelated(SchemaNode node, Adaptation dataSet) {
		...
		if (tableRef != null) {
			...
			// During schema compilation, it can't be guaranteed what order things are
			// compiled in,
			// and therefore it's expected that you can not yet know the link properties and
			// it can be null.
			if (link == null)
				return null;
	...
	/**
	 * Given a record and function that will return a parent record for a given
	 * record, and an optional function to get the display-name for a given record
	 * (label is used by default), traverse the parent path implied by the getParent
	 * function and return a string representation of that path if it creates a
	 * cycle, null otherwise.
	 * 
	 * @param record
	 * @param recordContext (for the record being modified, used to evaluate the
	 *                      first parent only)
	 * @param getParent     - binary function to get parent from record or context
	 *                      (pref to context)
	 * @param toString
	 * @return a string representing the path that creates the cycles
	 * @deprecated Use
	 *             {@link #detectAncestorCycle(Adaptation, ValueContext, BinaryFunction, UnaryFunction)}
	 *             or {@link #isAncestorOf(PrimaryKey, Adaptation, BinaryFunction)}
	 *             instead. This method is kept for backwards-compatibility, but it
	 *             doesn't check all potential parent paths for cycles. If the
	 *             record has a direct foreign key to its parent, it should still
	 *             work okay, but if it is over a link table, there are scenarios
	 *             where it won't catch the cycle.
	 */
	@Deprecated
	public static String detectCycle(Adaptation record, ValueContext recordContext,
	...
	/**
	 * Given a record and function that will return parent records for a given
	 * record, and an optional function to get the display-name for a given record
	 * (label is used by default), traverse the parent path implied by the
	 * getParents function and return a string representation of that path if it
	 * creates a cycle, null otherwise.
	 * 
	 * @param record
	 * @param recordContext (for the record being modified, used to evaluate the
	 *                      first parents only)
	 * @param getParents    - binary function to get parents from record or context
	 *                      (pref to context)
	 * @param toString
	 * @return a string representing the path that creates the cycles
	 */
	public static String detectAncestorCycle(Adaptation record, ValueContext recordContext,
		...
		List<Adaptation> cycleRecords = doDetectAncestorCycle(pk, parents, getParents, new ArrayList<>());
		if (cycleRecords != null) {
			...
			for (int i = 0; i < cycleRecords.size(); i++) {
				...
				// (like when creating a join record with the same record as both parent &
				// child).
				...
				if (!(i == 0 && record != null
						&& record.getOccurrencePrimaryKey().equals(cycleRecord.getOccurrencePrimaryKey()))) {
					if (message.length() > 0) {
						message.append("->");
					}
	...
	/**
	 * Return whether or not the given primary key is in the ancestor tree of
	 * another record. This utilizes similar logic to
	 * {@link #detectAncestorCycle(Adaptation, ValueContext, BinaryFunction, UnaryFunction)}
	 * but it can be used when you're comparing a record to see if it's the parent
	 * of another record, which can be used for table filters, versus checking that
	 * a record has itself as a parent.
	 * 
	 * @param recordPrimaryKey the primary key of the record to check
	 * @param otherRecord      the other record to check the ancestry on
	 * @param getParents       binary function to get parents from the other record
	 *                         (can use same function as
	 *                         <code>detectAncestorCycle</code>)
	 * @return whether or not it's an ancestor
	 */
	public static boolean isAncestorOf(PrimaryKey recordPrimaryKey, Adaptation otherRecord,
			BinaryFunction<Adaptation, ValueContext, List<Adaptation>> getParents) {
		List<Adaptation> parents = new ArrayList<>();
		parents.add(otherRecord);
		List<Adaptation> cycleRecords = doDetectAncestorCycle(recordPrimaryKey, parents, getParents, new ArrayList<>());
		return cycleRecords != null && !cycleRecords.isEmpty();
	}

	// This method recursively calls itself at every level up the tree, returning a
	// list of records representing
	// a cycle, or null if there's no cycle. The existing potential children of the
	// cycle is passed into it, and
	// in that way it builds up what the cycle will be if detected. This is needed
	// because a record can have
	// multiple parents and we need to know which path this invocation went up.
	private static List<Adaptation> doDetectAncestorCycle(PrimaryKey recordPrimaryKey, List<Adaptation> parentRecords,
			BinaryFunction<Adaptation, ValueContext, List<Adaptation>> getParents, List<Adaptation> cycleChildren) {
		...
		if (parentRecords != null) {
			...
			while (returnVal == null && iter.hasNext()) {
				...
				// itself, then the calls to its siblings would get messed up.
					...
					returnVal = doDetectAncestorCycle(recordPrimaryKey, nextParents, getParents, newCycleChidren);
				}
			}
		}
		return returnVal;
	...
	public static enum CompareOper {
		Equals, NotEquals, LessThan, LessThanOrEqual, GreaterThan, GreaterThanOrEqual, IsNull, IsNotNull, DateEarlier,
		DateEqual, DateLater, StartsWith, EndsWith, Contains, EqualsIgnoreCase
	};

	public static String createQueryPredicate(SchemaNode tableNode, Path[] paths, Object[] values,
		...
		if (compareOpers != null && compareOpers.length != paths.length)
			throw new IllegalArgumentException("If compareOpers is provided, but have one for each path/value");
		StringBuilder sb = new StringBuilder();
		for (int i = 0; i < paths.length; i++) {
			if (i > 0)
				sb.append(" and ");
			sb.append("(");
			appendPredicate(sb, tableNode, paths[i], values[i], compareOpers == null ? null : compareOpers[i]);
			sb.append(")");
		}
		return sb.toString();
	}

	public static String createQueryPredicate(SchemaNode tableNode, Path path, Object value, CompareOper compareOper) {
		StringBuilder sb = new StringBuilder();
		appendPredicate(sb, tableNode, path, value, compareOper);
		return sb.toString();
	}

	public static void appendPredicate(StringBuilder sb, SchemaNode tableNode, Path path, Object value,
	...
	public static void appendIdListPredicate(StringBuilder predicate, SchemaNode tableNode, Path path,
			List<String> idList, CompareOper compareOper) {
	...
	public static Object convertStringToValueObject(SchemaNode node, String value) throws ParseException {
		if (value == null) {
			return null;
		}
		SchemaTypeName type = node.getXsTypeName();
	 ...
	 *             Invoking this method is equivalent of calling that with
	 *             <code>true</code> as the last parameter, but you don't always
	 *             want to encode because if you know a field will never contain
	 *             special characters then you can avoid the extra processing.
	 */
	@Deprecated
	...
	private static void buildBinaryFunction(StringBuilder bldr, Path path, String value, String function) {
	...
	/**
	 * Given a table and a path to the 'name' field, determine whether the given
	...
	public static Adaptation getOneRecordOrThrowOperationException(final Repository aRepository,
			final String aBranchName, final String aAdapationName, final String aXpath) throws OperationException {
	...
	public static AdaptationHome getDataSpaceOrThrowOperationException(final Repository aRepository,
	...
	public static Adaptation getDataSetOrThrowOperationException(final AdaptationHome dataSpaceRef,
	...
	public static Adaptation getDataSetOrThrowOperationException(final Repository aRepository,
			final String dataSpaceName, final String dataSetName) throws OperationException {
		AdaptationHome dataSpaceRef = getDataSpaceOrThrowOperationException(aRepository, dataSpaceName);
		return getDataSetOrThrowOperationException(dataSpaceRef, dataSetName);
	}
	 ...
	 * @author MCH
	 *
	 *         Get a data set from a ValueContext, a data space name and a data set
	 *         name.
	 *
	 ...
	 *         searched.
	 * @param valueContext  The value context
	 * @param dataSpaceName the data space where to find the data set, null if in
	 *                      current data space
	 * @param dataSetName   the data set to find, if null current data set name will
	 *                      be searched.
	 * @return the data set
	 * @throws OperationException if data space or data set not found
	 */
	public static Adaptation getDataSetOrThrowOperationException(final ValueContext valueContext,
			final String dataSpaceName, final String dataSetName) throws OperationException {
		...
		if (dataSetName != null) {
			...
			if (instance == null) {
				throw OperationException
						.createError("Data set '" + dataSetName + "' not found in data space '" + dataSpaceName);
			}
		} else {
	...
	public static Adaptation getRecordByPrimaryKey(final Repository repository, final String dataspaceName,
			final String datasetName, final Path tablePath, final PrimaryKey primaryKey) throws OperationException {
	...
	public static Adaptation getRecordByPrimaryKey(final AdaptationHome dataspaceOrSnapshot, final String datasetName,
			final Path tablePath, final PrimaryKey primaryKey) throws OperationException {
		Adaptation dataset = AdaptationUtil.getDataSetOrThrowOperationException(dataspaceOrSnapshot, datasetName);
		AdaptationTable table = dataset.getTable(tablePath);
		return table.lookupAdaptationByPrimaryKey(primaryKey);
	}

	/**
	 * Within the same table as the provided record context, find the other records
	 * that match the paths/values provided. If the values provided is null, derive
	 * them from the recordContext
	 * 
	 * @param recordContext
	 * @param paths
	 * @param values
	 ...
	 */
	public static List<Adaptation> fetchRecordsMatching(ValueContext recordContext, List<Path> paths,
			List<Object> values) {
	 ...
	 */
	public static List<Adaptation> fetchRecordsMatching(ValueContext recordContext, List<Path> paths,
			List<Object> values, Path sortField) {
		...
		if (CollectionUtils.isEmpty(paths)) {
			...
			String predicate = AdaptationUtil.createQueryPredicate(tableOccNode, paths.toArray(new Path[0]),
					values.toArray(new Object[0]), null);
			if (sortField != null)
	 ...
	 * 
	 * The given request result will not be closed after processing, since the full
	 * context of the request result isn't known here. It is expected that the
	 * caller will close the request result, typically in a <code>finally</code>
	 * block.
	 * 
	...
	/**
	 * Given a record's valueContext and a list of paths, return a list of values
	...
	/**
	 * Given a value context representing a record occurrence, return the primary
	 * key string of that record.
	 * 
	 * @param recordContext
	...
	/**
	 * Given a table and path/object pairs, query a return the first matching
	 * record.
	 * 
	 * @param table
	 * @param paths
	 ...
	 */
	public static Adaptation getFirstMatching(AdaptationTable table, Path[] paths, Object[] values,
			CompareOper[] compareOpers) {
		// TODO: Can we use table.lookupFirstRecordMatchingPredicate? It does the same
		// thing.
	...
	/**
	 * Given the reference information (e.g. from a tableRef node), and a current
	 * data set, find the related table.
	 * 
	 * @param dataSet
	 * @param dataSpaceKey
	 * @param dataSetKey
	 ...
	 */
	public static AdaptationTable getTable(Adaptation dataSet, HomeKey dataSpaceKey, AdaptationName dataSetKey,
			Path tablePath) {
		AdaptationHome dataSpace = dataSet.getHome();
		if (dataSpaceKey != null) {
			dataSpace = dataSpace.getRepository().lookupHome(dataSpaceKey);
	 ...
	 */
	public static AdaptationTable getTable(final AdaptationHome dataSpace, final String dataSetName,
			final Path tablePath) throws OperationException {
		Adaptation instance = getDataSetOrThrowOperationException(dataSpace, dataSetName);
		if (instance == null) {
			throw OperationException
					.createError("Data set '" + dataSetName + "' not found in data space '" + dataSpace);
		}
	...
	/**
	 * Create a list of group objects (for a repeating group) given a schema node
	 * (must be a TNode) and an array of object arrays
	 *
	 * @param node   the schema node representing the repeating group
	 * @param groups the array of arrays of values
	 */
	public static List<GenericComplexObject> createRepeatingGroup(SchemaNode node, Object[]... groups) {
	...
	/**
	 * Given a record context and a list field node, answer whether the list field
	...
	/**
	 * Given a record context and a list field node, answer whether the list field
	 * value has duplicates allow option for lists containing String values to
	...
	/**
	 * Given a record, a field node representing a foreign key, and an optional
	 * filter (well formed xpath fk-filter), return the RequestResult of performing
	 * the query against the fk-table applying the filter (could have local record
	 ...
	 */
	public static RequestResult performFKFilterQuery(Adaptation record, SchemaNode fkNode, String xpathForRef) {
		...
		while (st.hasMoreTokens()) {
			...
			if (nextToken.startsWith("${")) {
				Path path = Path.parse(StringUtils.substringBefore(StringUtils.substringAfter(nextToken, "${"), "}"));
	...
	public static Adaptation findOrCreate(ProcedureContext pContext, AdaptationTable table, Path[] matchPaths,
			Map<Path, Object> createMap) throws OperationException {
	...
	public static Adaptation findOrDuplicate(ProcedureContext pContext, AdaptationTable table, Path[] matchPaths,
			Map<Path, Object> dupeMap, Adaptation origRecord) throws OperationException {
	...
	private static Adaptation find(AdaptationTable table, Path[] matchPaths, Map<Path, Object> createMap) {
		boolean key = false;
		Object[] matchValues;
		if (matchPaths == null) {
	...
	/**
	 * Given a path to a comparable field, return a comparator to sort by that field
	 * value
	 */
	...
	/**
	 * Given a path to a field and a value, return a predicate that tests whether
	 * the Adaptation's value for that field matches the value specified.
	 */
	...
	/**
	 * An import component of any 'refactor' effort. Essentially, take all
	 * references to the old object and redirect to the new object. Specify if you
	 * want to copy and leave the old value also, or if you are planning to delete
	 * the old one and you want to clean up.
	 * 
	 * @param pContext  a procedure context in which to create/delete records
	 * @param fromTable AdaptationTable with potential records pointing to old
	 *                  record
	 ...
	 */
	public static void redirectRefs(ProcedureContext pContext, AdaptationTable fromTable, Path fkPath,
			Adaptation oldRecord, Adaptation newRecord, boolean delete) throws OperationException {
		Path[] keySpec = fromTable.getPrimaryKeySpec();
		boolean key = CollectionUtils.indexInArray(keySpec, fkPath.getSubPath(1)) >= 0;
		RequestResult rr = fromTable.createRequestResult(AdaptationUtil.createQueryPredicate(
				fromTable.getTableOccurrenceRootNode(), fkPath, oldRecord.getOccurrencePrimaryKey().format(), null));
		try {
	 ...
	 *
	 * @author MCH
	 * @param pAdaptation either a record or an instance
	 * @param pPath       absolute path from the adaptation root
	...
	public static Map<AdaptationTable, String> deriveTableNames(Collection<AdaptationTable> tables, String separator,
			Locale locale) {
		...
		for (AdaptationTable table : tables) {
			...
			if (tablesWithName == null) {
				...
				String qualName = table.getTableNode().getParent().getLabel(locale) + separator + simpleName;
				result.put(table, qualName);
				for (AdaptationTable otherTable : tablesWithName) {
					String name = result.get(otherTable);
					if (!name.contains(separator)) {
						qualName = otherTable.getTableNode().getParent().getLabel(locale) + separator + simpleName;
	...
	/**
	 * Return the history records related to the given record. If history is not
	...
	public static List<Adaptation> getHistory(Adaptation record) {
		...
		RequestResult rr = historyTable.createRequestResult(
				createQueryPredicate(historyTable.getTableOccurrenceRootNode(), keySpec, keyVals, null));
		try {
	...
	public static String getRecordURL(Adaptation record) {
		...
		String ebxUrl = workflowAdmin.getString(WfiPaths._TechnicalConfiguration_Mail_UrlDefinition_Url);
		if (ebxUrl == null) {
		...
		urlBuilder.append("&instance=").append(record.getContainer().getAdaptationName().getStringName());
	...
	/**
	 * For a field path representing an inherited field, find the related record and
	 * return the value of the inherit from node.
	 * 
	 * @param record
	 * @param inheritField
	...
	public static Object getInheritValue(Adaptation record, Path inheritField) {
		...
		SchemaInheritanceProperties inhProps = recordNode.getNode(inheritField).getInheritanceProperties();
	...
	/**
	 * Get the records that refer to the given record. This is done regardless of
	 * whether the current user has permission to see those records.
	 * 
	 * @see SchemaNode#getRelationships()
	 * 
	 * @param record                     the record
	 * @param includeExternalDataSets    whether to include data sets other than the
	 *                                   one the record is in when looking for
	 *                                   references
	 * @param includedExternalDataSpaces a list of data spaces to limit the search
	 *                                   to, in addition to the data space that the
	 *                                   record is in, or <code>null</code> to
	 *                                   search all data spaces. For example, if the
	 *                                   record is potentially referenced by records
	 *                                   in data spaces A, B, and a child data space
	 *                                   of B, and only B is specified, then A and
	 *                                   the child of B will not be searched. The
	 *                                   record's own data space can be included as
	 *                                   well: If the record might be referenced by
	 *                                   the master data space of the record's data
	 *                                   space, or if another data set in the same
	 *                                   data space could reference the record, then
	 *                                   you may want to list the master data space.
	 *                                   Keep in mind, performance could be impacted
	 *                                   if there are many data spaces and all are
	 *                                   being searched (i.e. <code>null</code> is
	 *                                   specified).
	 * @return the set of records referencing the given record
	 */
	public static Set<Adaptation> getReferrers(Adaptation record, boolean includeExternalDataSets,
	...
	/**
	 * Get the first record found that references the given record, or
	 * <code>null</code> if there are no references. There is no guarantee to the
	 * order of searching for records, so it should not be assumed that in multiple
	 * invocations, the same record will be found first each time.
	 * 
	 * This is logically equivalent to calling
	 * {@link #getReferrers(Adaptation, boolean, Set)} and reading the first record
	 * from the returned set, however this is much more efficient because it can
	 * stop searching as soon as it finds one, rather than fill the whole set in.
	 * 
	 * If all you want to know is "Does any record refer to this one?" then you
	 * should call this method and check if result is <code>null</code>. The
	 * returned record can provide some info to include in an error message to the
	 * user to give a hint as to what is referring to the record, although it won't
	 * provide the complete picture of everything referencing it.
	 * 
	 * @param record                     the record
	 * @param includeExternalDataSets    whether to include data sets other than the
	 *                                   one the record is in when looking for
	 ...
	 *                                   being searched (i.e. <code>null</code> is
	 *                                   specified).
	 * @return the first found record referencing the given record, or
	 *         <code>null</code> if none are found
	 */
	public static Adaptation getFirstFoundReferrer(Adaptation record, boolean includeExternalDataSets,
			Set<String> includedExternalDataSpaces) {
		Set<Adaptation> referrers = doGetReferrers(true, record, includeExternalDataSets, includedExternalDataSpaces);
	...
	private static Set<Adaptation> doGetReferrers(boolean returnFirstFound, Adaptation record,
			boolean includeExternalDataSets, Set<String> includedExternalDataSpaces) {
		...
		Iterator<ReverseRelationshipIntraDataset> intraDataSetRelationshipIter = relationships
				.getIntraDatasetReverseRelationships().iterator();
		while (!doneFindingReferrers(referrers, returnFirstFound) && intraDataSetRelationshipIter.hasNext()) {
			ReverseRelationshipIntraDataset intraDataSetRelationship = intraDataSetRelationshipIter.next();
			if (isReverseRelationshipForeignKeyToTable(intraDataSetRelationship, tableNode)) {
				ReverseRelationshipResult relationshipResult = intraDataSetRelationship.getReferringRecords(record);
		...
		if (includeExternalDataSets && !doneFindingReferrers(referrers, returnFirstFound)) {
			Iterator<ReverseRelationshipInterDataset> interDataSetRelationshipIter = relationships
					.getInterDatasetReverseRelationships().iterator();
			while (!doneFindingReferrers(referrers, returnFirstFound) && interDataSetRelationshipIter.hasNext()) {
				ReverseRelationshipInterDataset interDataSetRelationship = interDataSetRelationshipIter.next();
				if (isReverseRelationshipForeignKeyToTable(interDataSetRelationship, tableNode)) {
					...
					// If the set of external data spaces to include is null, then it means we're
					// searching all data spaces
					if (includedExternalDataSpaces == null) {
						Iterator<ReverseRelationshipResultInDataspace> relationshipResultByDataSpaceIter = interDataSetRelationshipResults
								.getResultsByDataspaces().iterator();
						while (!doneFindingReferrers(referrers, returnFirstFound)
							...
							ReverseRelationshipResultInDataspace relationshipByDataSpaceResult = relationshipResultByDataSpaceIter
									.next();
							referrers.addAll(getReferrersForDataSpace(relationshipByDataSpaceResult, returnFirstFound));
						}
					}
					// Otherwise we are only looking at that list of data spaces
						...
						while (!doneFindingReferrers(referrers, returnFirstFound) && dataSpaceIter.hasNext()) {
							...
							if (repo.lookupHome(dataSpaceKey) == null) {
								// It's possible that the specified data space doesn't exist. That likely is a
								// developer error,
								// but there certainly are no references from a non-existent data space, so
								// don't think should
								// error out the whole method call. Just print a warning.
								LoggingCategory.getKernel().warn("Data space " + dataSpace
										+ " is specified as a data space to search for referrers, but is not found.");
							} else {
								// Get just the relationships for the current data space from the set that we're
								// iterating over
								...
								if (relationshipByDataSpaceResult != null) {
									referrers.addAll(
											getReferrersForDataSpace(relationshipByDataSpaceResult, returnFirstFound));
								}
							}
						}
					}
	...
	// Reverse relationships include records that reference this record at all,
	// including
	...
	private static boolean isReverseRelationshipForeignKeyToTable(ReverseRelationship relationship,
		...
		// If the reverse relationship's explicit relationship has an owner node that is
		// a foreign
	...
	private static Set<Adaptation> getReferrersForDataSpace(
			ReverseRelationshipResultInDataspace relationshipResultInDataSpace, boolean returnFirstFound) {
		Set<Adaptation> referrers = new LinkedHashSet<>();
		Iterator<ReverseRelationshipResult> resultByDataSetIter = relationshipResultInDataSpace.getResultByDatasets()
				.iterator();
	...
	// If returnFirstFound is true, the resulting set will have at most one record.
	private static Set<Adaptation> getReferrersForDataSet(ReverseRelationshipResult relationshipResult,
	...
	/**
	 * Given a data set, return the updated and/or created records compared to its
	 * initial snapshot, for all tables in the data set.
	 * 
	 * This is equivalent to calling {@link #getChangedRecords(Adaptation, Set)}
	 * with <code>null</code> for the table paths.
	 * 
	 * @param dataset the data set
	...
	/**
	 * /** Given a data set, return the updated and/or created records compared to
	 * its initial snapshot, for the specified tables in the data set.
	 * 
	 * This is equivalent to calling
	 * {@link #getChangedRecords(Adaptation, Set, boolean)} with <code>false</code>
	 * for resolvedMode.
	 * 
	 * @param dataset    the data set
	 * @param tablePaths the table paths, or <code>null</code> if it should include
	 *                   all tables
	 * @return list of updated/created records for the data set
	 */
	public static List<Adaptation> getChangedRecords(Adaptation dataset, Set<Path> tablePaths) {
		return getChangedRecords(dataset, tablePaths, false);
	}

	/**
	 * Given a data set, return the updated and/or created records compared to its
	 * initial snapshot. If table paths are specified, only return changes in those
	 * tables.
	 * 
	 * @param dataset      the data set
	 * @param tablePaths   the table paths, or <code>null</code> if it should
	 *                     include all tables
	 * @param resolvedMode use false for best performance, use true if inheritance
	 *                     and computed attributes need to be taken into account
	 * @return list of updated/created records for the data set
	 */
	public static List<Adaptation> getChangedRecords(Adaptation dataset, Set<Path> tablePaths, boolean resolvedMode) {
		...
		DifferenceBetweenInstances diffs = DifferenceHelper.compareInstances(dataset, initDataset, resolvedMode);
		...
		for (DifferenceBetweenTables tableDiff : tableDiffs) {
			...
			if (tablePathOnLeft == null || tablePaths == null || tablePaths.contains(tablePathOnLeft)) {
	...
	/**
	 * Given a data set, return the deleted records compared to its initial
	 * snapshot. The deleted records will be in the initial snapshot's data set
	 * (since they don't exist in child).
	 * 
	 * This is equivalent to calling {@link #getDeletedRecords(Adaptation, Set)}
	 * with <code>null</code> for the table paths.
	 * 
	 * @param dataset the data set
	...
	public static List<Adaptation> getDeletedRecords(Adaptation dataset, Set<Path> tablePaths) {
		...
		DifferenceBetweenInstances diffs = DifferenceHelper.compareInstances(dataset, initDataset, false);
		if (diffs == null || diffs.isEmpty())
		...
		for (DifferenceBetweenTables tableDiff : tableDiffs) {
			...
			if (tablePathOnRight == null || tablePaths == null || tablePaths.contains(tablePathOnRight)) {
